...
| A complete semantic guard over documentation | The authoritative control is the closed allowlist in `spec/authority-policy.json`, which fails closed on any statement naming a governed document. The prose scanner beside it is advisory and covers only shapes already known to have shipped. |
