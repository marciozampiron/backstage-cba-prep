...
that: `X123`, and a literal space, pass just as well. GNU `date` then parses the result, so the
value flows on.

It bypasses nothing. Expiry and the 12-hour ceiling are computed from the parsed instant and still
hold. What it breaks is the closed schema: the artifact was laxer than the Stage A regex it exists
to mirror, and a divergence between the two validators of the same field is a governance defect
whatever the arithmetic downstream does.

Why the suite did not catch it: the schema test asserted on the script TEXT, and only on the tail of
the pattern — `(Z|[+-][0-9]{2}:[0-9]{2})$`. The fraction group could degrade to anything and the
assertion stayed green. Asserting on generated text proves the generator emitted text.

The replacement runs the generated bytes. The pattern is EXTRACTED from the artifact rather than
copied — a copy proves only that the copy is correct — and exercised on its own against a matrix:
`Z`, dot-fraction and numeric offset accepted; comma, letter, space, empty fraction, missing zone
and a bare word refused. Each case is then compared against Stage A's format verdict, so the two
validators are asserted to agree rather than assumed to.

Discrimination: with `\\.` reverted to `\.`, the focused validator test fails on the exact value
Codex reported and the end-to-end gate table fails alongside it — **2** failures, 92 passing. The
generator file was restored byte-identical after the proof.
