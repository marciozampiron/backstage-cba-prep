...
| SPEC-DEPLOY-002 | RETIRED (never ACTIVE; `supersededBy` SPEC-DEPLOY-019) | The cloud authorization value satisfies a closed ten-key schema with `issue` pinned to 70, `mode` in {plan_only, deploy} and a `decisionId` matching its documented shape; any other shape refuses. | — | — | absorbed before activation (§4) in I4: the tree now implements the complete successor schema, and an id describing code that no longer exists cannot stay PROPOSED honestly |
...
| SPEC-DEPLOY-009 | ACTIVE | An authorization instant is accepted only as strict RFC3339 UTC that round-trips through the calendar; a normalized or offset instant refuses. | `infra/aws/bin/deploy-release.js` (`strictUtcInstant`) | `infra/aws/test/deploy-preflight.test.js` | ACTIVATED in batch 2 (#115): dedicated adversarial suite named; §6c record at 30c1da85 — a lenient parser fails exactly the named test |
| SPEC-DEPLOY-010 | ACTIVE | An authorization is valid only while `approvedAt` ≤ now < `expiresAt`, with `expiresAt − approvedAt` at most one hour. | `infra/aws/bin/deploy-release.js` (`CLOUD_GATE_MAX_TTL_MS`) | `infra/aws/test/deploy-preflight.test.js` | ACTIVATED in batch 2 (#115): dedicated adversarial suite named; §6c record at 30c1da85 — a doubled TTL ceiling fails exactly the named test |
...
| SPEC-DEPLOY-016 | ACTIVE | A plan recomputed at execution time that differs from the digest the authorization names refuses as `PLAN_CHANGED`, and no change set executes. | `infra/aws/bin/deploy-release.js` | `infra/aws/test/deploy-preflight.test.js` | ACTIVATED in I8: conformance runs the two named PLAN_CHANGED tests; §6c record at cee1ac66 — the deploy-path refusal removed fails exactly the named test |
...
| SPEC-DEPLOY-019 | PROPOSED (absorbed -002 and -020) | The cloud authorization value has EXACTLY these eleven keys — `issue`, `mode`, `decisionId`, `releaseSha`, `environment`, `manifestDigest`, `stacks`, `planDigest`, `absentEntryDigests`, `approvedAt`, `expiresAt` — no key absent and none unknown, with the per-key and per-mode constraints of §8a. Any other shape, and any effect outside the value's `mode`, refuses. | `infra/aws/bin/deploy-release.js` (`checkCloudGate`, `CLOUD_GATE_KEYS`) | `infra/aws/test/deploy-preflight.test.js` | code + tests reviewed in I4: manifestDigest recomputed from the verified manifest (never trusted), the retired key refused as unknown, abandon-mode gates schema-valid but refused ABANDON_NOT_IMPLEMENTED before any change-set API call or mutation (identity reads precede the gate check by design) until the abandon lane lands |
...
| SPEC-DEPLOY-021 | ACTIVE | Deleting the empty stack record a CREATE change set leaves behind is an effect distinct from deleting a change set, authorized by its own out-of-band instrument (§8b) that no lane can read, and no automated lane performs it: `DeleteStack` accepts no expected-status precondition, so an observed `REVIEW_IN_PROGRESS` cannot constrain the delete that follows it, and the release concurrency lock binds only this repository's lanes. A lane that would delete a stack record refuses; the condition is reported and resolved by a separate human decision. | `spec/authority-policy.json` (`delete-review-in-progress-stack-record`), `infra/aws/bin/deploy-release.js` (abandon) | `test/governance-model.test.js`, `infra/aws/test/deploy-preflight.test.js` | ACTIVATED in I8 (traceability completed in I8-2): conformance runs the policy-data test and the abandon meta-test; §6c record at cee1ac66 — an inserted DeleteStack call fails exactly the named test |
| SPEC-LANE-005 | PROPOSED | A `bind_only` dispatch terminates after the preflight and is structurally unable to enter a stage that prepares or executes change sets, whatever the Environment holds at any moment of the run. | `.github/workflows/release-pilot.yml` (`bind-stage`) | `test/release-pilot-workflow.test.js` | code + tests reviewed in I2: mode-gated on the immutable dispatch input, no id-token, no OIDC consumer, DAG-terminal |
| SPEC-LANE-006 | PROPOSED | Every dispatch carries a caller-generated correlation id matching exactly `^cba-70-[0-9a-f]{32}$`; a dispatch whose id does not match is refused in the preflight, before any credentialed stage. The run's NAME is exactly `cba-release <mode> <correlationId>` and nothing else, so a run is selected by EQUALITY on its complete name — never by substring — and is identifiable from run metadata alone, before any artifact exists. The same id appears inside the structured uploaded artifact, together with the release SHA the run acted on, which is what a reviewer compares against the request; a run's `headSha` is the dispatch ref's tip and is never that comparison. | `.github/workflows/release-pilot.yml` (`run-name`, `correlation_id`, `bind-stage`) | `test/release-pilot-workflow.test.js` | code + tests reviewed in I2: closed-grammar refusal executed against the real script; run name and binding artifact pinned |
| SPEC-LANE-007 | PROPOSED | Run resolution is bounded and unambiguous: the workflow is pinned by FILE identity inside the helper (`release-pilot.yml` — a display name is not an identity, and no caller may supply another); the query pins `--branch main` and `--event workflow_dispatch`, matches the complete run name by EQUALITY, and is EXHAUSTIVE OR STOPS — a page that returns as many rows as requested refuses as truncated rather than proving uniqueness from the newest slice. It polls at most ten times with thirty seconds between attempts and no wait after the last, stops on zero matches after the bound or more than one at ANY query, and every external call carries a reviewed wall-clock deadline with a named timeout stop — ten attempts do not bound a stalled call or an endlessly queued run. After the run reaches a terminal conclusion, the SAME query must re-observe exactly the same single run id immediately before the artifact is accepted. | `bin/resolve-run.mjs` | `test/resolve-run.test.js` | helper + simulated-`gh` tests reviewed in this commit; the run name itself awaits the workflow |
...
| SPEC-LANE-001 | PROPOSED | In every credentialed job, synthesis completes before the pinned OIDC consumer step, and after that step ONLY the reviewed entrypoints and the evidence-output reader execute — no action step and no package-manager command, ever, because `id-token: write` is job-scoped and no scrub can remove the ability to mint a fresh token. Evidence leaves the lane exclusively from a job that never holds `id-token`. | `.github/workflows/release-pilot.yml` | `test/release-pilot-workflow.test.js` | code + tests reviewed, reversion proven (I3-2 replaced the short-lived scrub clause with the job boundary after review showed env scrubbing cannot close OIDC authority) |
...
| SPEC-LANE-004 | PROPOSED | The dispatch input `mode` offers only the reviewed non-pilot set (`bind_only`, `dev_only`, `abandon`), so the pilot jobs' success expressions are unreachable. | `.github/workflows/release-pilot.yml` | `test/release-pilot-workflow.test.js` | code + tests reviewed, reversion proven (text widened in I2 and I5 while PROPOSED — §4 permits it) |
...
The value is a JSON object with EXACTLY these eleven keys — none absent, none unknown, compared
as a sorted key set:
...
| `manifestDigest` | string matching `/^[0-9a-f]{64}$/` — the §6b `bundle` digest over the CANONICAL (deep-key-sorted) serialization of the complete manifest, envelope pinned in `infra/aws/lib/deploy-preflight.js` (`manifestBundleDigest`: producer `cba-release-binding`, record name `binding-manifest`, `application/json`); the binding run computes it, the entrypoint RECOMPUTES it from the verified manifest at the gate, and `test/digest-agreement.test.js` proves the ESM/CJS framings identical | required, never null | required | equal to the plan decision's | equal to the plan decision's |
...
| `absentEntryDigests` | `null`, or a non-empty array of DISTINCT strings each matching `/^[0-9a-f]{64}$/` — the per-entry digests of the already-deleted prefix of a partially-abandoned plan, one per absent set, consumed in group order (round I5-2) | abandon-only; every other mode requires exactly `null` | **exactly `null`** | **exactly `null`** | `null` for a fresh abandon; the continuation list when resuming a partial abandon under a NEW decision |
...
fails its regex, an empty `stacks`, a `stacks` outside the reviewed groups, a `planDigest`
whose nullability contradicts the mode, or an `absentEntryDigests` outside abandon mode, empty,
duplicated or malformed.

**The plan digest is a ROOT, and a partial abandon RESUMES under it (round I5-2).** `planDigest`
commits to the ordered LIST of per-entry digests (each entry digested alone, then the list
digested) — equal in strength to digesting the entries wholesale, and decomposable on purpose:
when an abandon stops mid-way (`ABANDON_DELETE_FAILED`, a lapsed window), the deleted prefix's
entry digests are already on the evidence record (`changeSets[].canonicalSha256`). A NEW abandon
decision carries them as `absentEntryDigests`; the run re-describes the group, recomputes each
PRESENT entry's digest, takes each ABSENT position's digest from the gate in order, and the SAME
root must emerge before anything is deleted. A recreated set, a foreign set, an absence with no
supplied digest, or a leftover digest each kill the root — `PLAN_CHANGED` or
`CHANGE_SET_MISSING`, and NOTHING is deleted. The stack records of the whole wave, the absent
prefix's included, are still REPORTED (SPEC-DEPLOY-021).

Round I5-3 closed the law: the absences of a genuine continuation form a **PREFIX** of the group
order — the lane deletes in order, so an absence after the first present entry is a state this
operation cannot have produced, and it refuses `ABANDON_NOT_A_PREFIX` with nothing deleted, even
when the supplied digests would close the root. Every continuation artifact is **self-sufficient
for the next decision**: its `planDigest` is the ORIGINAL root (never the present-subset digest)
and its `changeSets[]` is the FULL ordered stack → `canonicalSha256` map, previously-absent
positions carried as `ALREADY_ABSENT` — so a second interruption, and any after it, resumes from
...
Round I5-4 closed the recording gap and round I5-5 closed the observation gap: a failed delete
CALL is an AMBIGUOUS outcome — the service may have accepted the deletion while the transport
died — so the lane reconciles by BOUNDED RE-OBSERVATION (five attempts) before recording
anything. A successful describe is NOT proof the delete was rejected: CloudFormation's official
statuses include `DELETE_PENDING`, `DELETE_IN_PROGRESS` and `DELETE_COMPLETE`, and an accepted
deletion passes through them before `ChangeSetNotFound`. Conclusive absence is
`ChangeSetNotFound` at any attempt. Conclusive presence (round I5-6) requires the WHOLE window
to testify — EVERY observation well-formed, identity-matched (`ChangeSetId`) and in a STANDING
status from the documented closed enum (`CREATE_PENDING`, `CREATE_IN_PROGRESS`,
`CREATE_COMPLETE`, `FAILED`, `DELETE_FAILED` — the last meaning the service tried and the set
REMAINS); a deletion glimpsed at any attempt is not erased by a calm final read, and a status
outside the enum is a fact the code cannot claim. Anything less — a deleting or unknown status
anywhere in the window, a malformed response, a diverging identity, a transport error — is
UNKNOWN. Provably absent: the deletion is recorded as `abandoned` (the
...
**Round 8: a boolean is not a decision. Round 9: a free-text record is not an enforceable one.**
`riskAccepted: false` could be flipped alongside `executableProcedure` in a single edit; and the
first record shape could have ridden on an unrelated finding, a different stack's decision, or an
expiry that existed only as prose. The policy carries `riskAcceptance: null`, and the validator
...
- `zampStatement` — a CLOSED pointer to **Zamp's verbatim written decision**: `source` (literally
  ...
  `zampStatementDigest(locator, content)` pins `producer: "zamp"`, record `name` =
  `<path>@<introducedIn>` — the COMPLETE locator, because round 12 showed that with only the
...
- `coversCleanupAuthorizationSha256` and `coversCleanupDecisionId` — an acceptance covers ONE
  ...
  `stack-record-authorization-value`, `mediaType: "application/json"`, `producer: "zamp"`.
  Round 12 made the shape a PRECONDITION: an input that is not a plain object with exactly those
  nine keys — an extra key, a missing one, a present-but-undefined value, a wrong type — REFUSES
  rather than digesting a projection, because a digest of what the function kept is not a digest
  of the value presented. (The full per-key grammar parser remains SPEC-DEPLOY-022's activation
  obligation: this function refuses shape; the parser will refuse content.) It is never a
  class-wide waiver;
- `acceptedAt`, `reviewBy`, `expiresAt` — strict UTC instants, ordered
  ...
  closed until Zamp renews the decision or the procedure reverts to non-executable;
- `boundToEffect` — an effect this instrument authorizes.
...
| A Zamp-authored `riskAcceptance` record (thirteen closed keys: digests of the exact residual and of Zamp's verbatim statement, one-stack/one-decision coverage, clock-evaluated expiry) before any executable cleanup procedure | A boolean flip is not a risk decision, and a free-text record is not an enforceable one; the runtime consumer must re-check expiry and coverage immediately before the effect | SPEC-DEPLOY-022 |
