...
`ChangeSetNotFound` at any attempt; conclusive presence is the FINAL attempt returning a
well-formed, identity-matched response in a NON-delete status; a deleting status, a malformed
response, a diverging `ChangeSetId` or a transport error at the bound is UNKNOWN. Provably absent: the deletion is recorded as `abandoned` (the
