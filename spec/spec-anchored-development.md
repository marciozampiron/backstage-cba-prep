...
| SPEC-LANE-004 | PROPOSED | The dispatch input `mode` offers only the reviewed non-pilot set (`bind_only`, `dev_only`), so the pilot jobs' success expressions are unreachable. | `.github/workflows/release-pilot.yml` | `test/release-pilot-workflow.test.js` | code + tests reviewed, reversion proven (text widened in I2 while PROPOSED — §4 permits it) |
