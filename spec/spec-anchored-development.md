...
| SPEC-LANE-001 | PROPOSED | In every credentialed job, synthesis completes before the pinned OIDC consumer step; after that step, until the reviewed credential scrub empties the AWS environment, only the reviewed entrypoints execute — no action step and no package-manager command — and after the scrub only the pinned evidence uploaders and the evidence read may run. | `.github/workflows/release-pilot.yml` | `test/release-pilot-workflow.test.js` | code + tests reviewed, reversion proven (text widened in I3 while PROPOSED — §4 permits it; the scrub closes the window before any uploader runs) |
