...
| SPEC-DEPLOY-019 | ACTIVE | The cloud authorization value has EXACTLY these eleven keys — `issue`, `mode`, `decisionId`, `releaseSha`, `environment`, `manifestDigest`, `stacks`, `planDigest`, `absentEntryDigests`, `approvedAt`, `expiresAt` — no key absent and none unknown, with the per-key and per-mode constraints of §8a. Any other shape, and any effect outside the value's `mode`, refuses. | `infra/aws/bin/deploy-release.js` (`checkCloudGate`, `CLOUD_GATE_KEYS`) | `infra/aws/test/deploy-preflight.test.js` | ACTIVATED in batch 2 (#115): dedicated adversarial suite named; §6c record at 30c1da85 — a subset key check fails exactly the named test; reciprocity with RETIRED -002/-020 preserved |
