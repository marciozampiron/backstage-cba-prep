...
function printConfig(cfg, check) {
  console.log(`\n  ${c.bold('Model config')} ${c.gray(`(backend: ${cfg.backend})`)}`);
  console.log(hr());
  if (cfg.backend === 'bedrock') {
    console.log(`  region:  ${cfg.region ? c.cyan(cfg.region) : c.red('unset')}`);
    console.log(`  profile: ${cfg.profile ? c.cyan(cfg.profile) : c.gray('(env credentials)')}`);
    console.log('');
  }
  for (const tier of TIERS) console.log(`    ${c.bold(tier.padEnd(9))} ${c.cyan(cfg.models[tier])}`);
  console.log('');
  for (const w of check.warnings) console.log(`  ${c.yellow('⚠')}  ${c.gray(w)}`);
  for (const e of check.errors) console.log(`  ${c.red('✗')}  ${e}`);
  console.log(hr());
}
...
export async function runBedrockCheck(opts = {}) {
  ...
  if (opts.json) {
    console.log(JSON.stringify({ config: cfg, check }, null, 2));
    return check.ok ? 0 : 1;
  }

  printConfig(cfg, check);
  ...
  if (!opts.smoke) {
    console.log(c.green('  ✓ Config valid') + c.gray('  (dry-run — no model was invoked, no tokens spent).'));
    console.log(c.gray('  Live smoke test (COSTS TOKENS): node bin/cli.js bedrock-check --smoke\n'));
    return 0;
  }
  ...
  console.log(`     tier ${c.bold(tier)} → ${c.cyan(modelId)} in ${c.cyan(cfg.region)}\n`);
  if (!opts.yes) {
    const answer = (await ask('  Type "yes" to proceed (anything else aborts): ')).trim().toLowerCase();
    closeInput();
    if (answer !== 'yes') {
      console.log(c.gray('  Aborted — no call made.\n'));
      return 0;
    }
  }
  ...
  if (/AccessDenied/i.test(msg)) {
    console.log(c.gray('    → request access: Bedrock console → Model access → enable Anthropic models.'));
  } else if (/on-demand|inference profile|ValidationException/i.test(msg)) {
    console.log(c.gray('    → use the us.* inference-profile id (aws bedrock list-inference-profiles), not the bare model id.'));
  } else if (/could not|not found|ENOENT|command not found/i.test(msg)) {
    console.log(c.gray('    → is the AWS CLI installed and on PATH?'));
  }
  if (msg) console.log(c.gray('    ' + msg.split('\n')[0].slice(0, 200)));
  console.log('');
  return 2;
}
