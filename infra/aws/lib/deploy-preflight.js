...
const CODES = {
  ...
  CHANGE_SET_DEPLOYMENT_CONFIG_ABSENT: 'has a change set whose description does not state its deployment configuration (mode and rollback-on-failure) — an authorization that does not say how it executes is incomplete, and absence is never read as the safe default',
  CHANGE_SET_DEPLOYMENT_CONFIG_UNSUPPORTED: 'has a change set whose deployment configuration is outside what this lane approves — only STANDARD mode with rollback-on-failure enabled is executable here; EXPRESS changes when resources count as complete and a disabled rollback leaves a half-applied stack standing, and neither is visible in the resource diff a human reads',
  CHANGE_SET_PAGES_DIVERGE: 'has a change set whose pages do not describe the same change set — the pagination merge keeps only the first page\'s executable semantics, so a later page that disagrees about identity, capabilities, failure behaviour or deployment configuration would be read and then discarded; a description that contradicts itself authorizes nothing',
